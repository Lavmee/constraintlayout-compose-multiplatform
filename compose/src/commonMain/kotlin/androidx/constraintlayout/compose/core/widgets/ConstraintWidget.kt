...
import androidx.constraintlayout.compose.core.Cache
import androidx.constraintlayout.compose.core.LinearSystem
import androidx.constraintlayout.compose.core.LinearSystem.Companion.DEBUG
import androidx.constraintlayout.compose.core.LinearSystem.Companion.FULL_DEBUG
import androidx.constraintlayout.compose.core.SolverVariable
import androidx.constraintlayout.compose.core.ext.Math
import androidx.constraintlayout.compose.core.state.WidgetFrame
import androidx.constraintlayout.compose.core.widgets.ConstraintAnchor.Type.BASELINE
import androidx.constraintlayout.compose.core.widgets.ConstraintAnchor.Type.BOTTOM
import androidx.constraintlayout.compose.core.widgets.ConstraintAnchor.Type.CENTER
import androidx.constraintlayout.compose.core.widgets.ConstraintAnchor.Type.CENTER_X
import androidx.constraintlayout.compose.core.widgets.ConstraintAnchor.Type.CENTER_Y
import androidx.constraintlayout.compose.core.widgets.ConstraintAnchor.Type.LEFT
import androidx.constraintlayout.compose.core.widgets.ConstraintAnchor.Type.NONE
import androidx.constraintlayout.compose.core.widgets.ConstraintAnchor.Type.RIGHT
import androidx.constraintlayout.compose.core.widgets.ConstraintAnchor.Type.TOP
import androidx.constraintlayout.compose.core.widgets.ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT
import androidx.constraintlayout.compose.core.widgets.ConstraintWidget.DimensionBehaviour.WRAP_CONTENT
import androidx.constraintlayout.compose.core.widgets.analyzer.ChainRun
import androidx.constraintlayout.compose.core.widgets.analyzer.HorizontalWidgetRun
import androidx.constraintlayout.compose.core.widgets.analyzer.VerticalWidgetRun
...
open class ConstraintWidget {
    ...
    fun setFinalHorizontal(x1: Int, x2: Int) {
        ...
        if (FULL_DEBUG) {
            println(
                "*** SET FINAL HORIZONTAL FOR $debugName : $x1 -> $x2 (width: $mWidth)",
            )
        }
    }
    ...
    fun setFinalVertical(y1: Int, y2: Int) {
        ...
        if (FULL_DEBUG) {
            println(
                "*** SET FINAL VERTICAL FOR $debugName : $y1 -> $y2 (height: $mHeight)",
            )
        }
    }
    ...
    fun hasResolvedTargets(orientation: Int, size: Int): Boolean {
        if (orientation == HORIZONTAL) {
            if (mLeft.mTarget != null && mLeft.mTarget!!.hasFinalValue() && mRight.mTarget != null && mRight.mTarget!!.hasFinalValue()) {
                return (
                    mRight.mTarget!!.getFinalValue() - mRight.margin -
                        (mLeft.mTarget!!.getFinalValue() + mLeft.margin)
                    ) >= size
            }
        } else {
            if (mTop.mTarget != null && mTop.mTarget!!.hasFinalValue() && mBottom.mTarget != null && mBottom.mTarget!!.hasFinalValue()) {
                return (
                    mBottom.mTarget!!.getFinalValue() - mBottom.margin -
                        (mTop.mTarget!!.getFinalValue() + mTop.margin)
                    ) >= size
            }
        }
        return false
    ...
    private var mIsWidthWrapContent = false
    private var mIsHeightWrapContent = false
    ...
    val maxHeight: Int get() = mMaxDimension[VERTICAL]

    val maxWidth: Int get() = mMaxDimension[HORIZONTAL]
    ...
    var isMeasureRequested: Boolean
        get() = mMeasureRequested && mVisibility != GONE
        set(value) {
    ...
    var mListAnchors: Array<ConstraintAnchor> =
        arrayOf(mLeft, mRight, mTop, mBottom, mBaseline, mCenter)
    ...
    val x: Int
        get() = if (mParent != null && mParent is ConstraintWidgetContainer) {
    ...
    fun resetAnchors() {
        ...
        if (parent != null && parent is ConstraintWidgetContainer) {
            val parentContainer: ConstraintWidgetContainer? =
                getParent() as ConstraintWidgetContainer?
    ...
    open fun addToSolver(system: LinearSystem, optimize: Boolean) {
        ...
        if (!(
                mVisibility != GONE || mAnimated || hasDependencies() ||
                ...
                if (horizontalParentWrapContent && mParent != null) {
                    if (mOptimizeWrapOnResolved) {
                        val container: ConstraintWidgetContainer =
                            mParent as ConstraintWidgetContainer
        ...
        if ((
            ...
            if (mParent != null) {
                if ((
                    ...
                    val parentMax = system.createObjectVariable(mParent!!.mRight)
                    system.addGreaterThan(
                        (parentMax)!!,
                        (right),
                        0,
                ...
                if ((
                    ...
                    val parentMax = system.createObjectVariable(mParent!!.mBottom)
                    system.addGreaterThan(
                        (parentMax)!!,
                        (bottom),
                        0,
        ...
        if (mParent != null) {
            ...
            if (!inHorizontalChain && horizontalParentWrapContent && (
                ...
                val parentRight = system.createObjectVariable(mParent!!.mRight)
            ...
            if (!inVerticalChain && verticalParentWrapContent && (
                ...
                val parentBottom = system.createObjectVariable(mParent!!.mBottom)
                ...
                if (matchConstraintDefault == MATCH_CONSTRAINT_WRAP) {
                    ...
                    if (beginAnchor.type == TOP ||
                        ...
                        percentBegin = system.createObjectVariable(
                            mParent!!.getAnchor(TOP),
                        )
                        percentEnd = system.createObjectVariable(
                            mParent!!.getAnchor(BOTTOM),
                        )
                    } else {
                        percentBegin = system.createObjectVariable(
                            mParent!!.getAnchor(LEFT),
                        )
                        percentEnd = system.createObjectVariable(
                            mParent!!.getAnchor(RIGHT),
                        )
                    }
        ...
        if (!isBeginConnected && !isEndConnected && !isCenterConnected) {
            ...
            if (parentWrapContent) {
                if (mOptimizeWrapO && begin!!.isFinalValue && (mParent != null)) {
                    val container: ConstraintWidgetContainer = mParent as ConstraintWidgetContainer
                    if (isHorizontal) {
    ...
    open fun copy(src: ConstraintWidget, map: HashMap<ConstraintWidget?, ConstraintWidget?>) {
        ...
        mParent = if (mParent == null) null else map[src.mParent]
        mWidth = src.mWidth
        mHeight = src.mHeight
